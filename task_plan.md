...
1. **模板封面预览图**：为每个模板生成封面缩略图，前端以图片形式展示模板设计。
2. **更多图片候选**：图片搜索接口返回 6-12 张候选图，提升选择空间。
3. **按方案推荐图片组**：PPT 页数较多时，按页面主题分组，每组推荐风格一致的一套图片。
4. **页面编辑语音修改**：集成 Web Speech API，支持语音输入修改标题和正文。

> 用户决定暂停推进，保留工作日志，下次继续。推荐先实现 1+2。
